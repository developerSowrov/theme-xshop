...
const Filters: FC<TProps> = ({
   ...
   return (
      <section
         className={`${
            isCloseFilter ? "top-0" : "top-full"
         } duration-300 fixed left-0 h-full w-full rounded-2xl border border-gray-200 bg-white p-5 transition-all max-xl:z-50 max-xl:shadow-xl xl:sticky xl:top-32 xl:w-1/4`}
      >
         {/* Title */}
         <div className="flex items-center justify-between">
            <Title title="Filters" />
            <Image src={filter} alt="filter" width={20} height={20} className="w-5 h-5 max-xl:hidden" />
